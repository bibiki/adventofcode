...
(defn first-op [program op1 op2 op3]
  (println "a" op1 op2 op3)
  (assoc program op3 (+ op1 op2)))

(defn second-op [program op1 op2 op3]
  (assoc program op3 (* op1 op2)))

(defn third-op [program operand]
  (println "thirdop" operand)
  (assoc program operand operand))

(defn fourth-op [program operand]
  (print (get program 50)))
...
(defn read-param [program mode param]
  (case mode
    0 (get program param)
    1 param))
...
(defn read-modes [input op-count]
  (println "opcount" op-count)
  (if (= 1 op-count)
    '(1)
    (concat
     (list (rem input 10))
     (read-modes (quot input 10) (dec op-count)))))

(defn get-function-operands [op-number x counter]
  (case op-number
    1 (take 3 (drop (inc counter) x))
    2 (take 3 (drop (inc counter) x))
    3 (take 1 (drop (inc counter) x))
    4 (take 1 (drop (inc counter) x))))

(defn part-first [program counter]
  (let [operation (get-operation (get program counter))]
    (println "operation" operation "counter" counter (get program counter))
    (if (= 99 operation)
      (get program 0)
      (let [op-function (get-op-function operation)
            function-operands (get-function-operands operation program counter)
            modes (read-modes (quot (get program counter) 100) (count function-operands))
            pair-mode-operand (map list modes function-operands)
            eval-operands (map #(read-param program (first %) (last %)) pair-mode-operand)
            update-program (apply op-function (concat (list program) eval-operands))
            ]
        (part-first update-program (+ counter 1 (count function-operands)))))))
